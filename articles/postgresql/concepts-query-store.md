...
This view returns query text data in Query Store. There is one row for each distinct query_text. The data isn't avaiable via the Intelligent Perofrmance section in the portal, API's, or the CLI - but It can be found by connecting to azure_sys and querying 'query_store.query_texts_view'.
